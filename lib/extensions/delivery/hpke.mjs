...
export const DELIVERY_SCHEMES = new Set([DELIVERY_SCHEME]);
const IV_BYTES = 12;
const TAG_BYTES = 16;
const HASH_BYTES = 32;
...
export function seal(plaintext, recipientPublicKeyB64, scheme = DELIVERY_SCHEME) {
  if (scheme !== DELIVERY_SCHEME) {
    throw new Error(`unsupported delivery ciphertext scheme: ${scheme}`);
  }
  ...
  const { key, nonce: iv } = deriveHpkeContext(
    deriveHpkeSharedSecret(shared, encRaw, rawPublicKey(recipientPublic)),
  );
...
export function open(sealedJson, recipientPrivateKeyB64) {
  ...
  if (parsed?.version !== 1 || parsed?.scheme !== DELIVERY_SCHEME) {
  ...
  const { key, nonce } = deriveHpkeContext(deriveHpkeSharedSecret(
    shared,
