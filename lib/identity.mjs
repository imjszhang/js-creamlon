...
const ROTATION_KEYS = new Set(['version', 'old_public_key', 'new_public_key', 'rotated_at', 'signature']);
...
export function inspectKeyContinuity(text, currentPublicKey, options = {}) {
  ...
  for (let i = 0; i < rotations.length; i += 1) {
    ...
    if (item.version !== PROTOCOL_VERSION) errors.push(`rotation ${i + 1}: invalid version`);
