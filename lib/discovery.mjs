...
export async function discoverRepositories(repositories, options) {
  ...
  const candidates = await mapLimit(repositories, 6, async (repository) => {
    ...
    try {
      ...
      const continuity = inspectKeyContinuity(rotationsText, agent.creamlon.public_key);
  ...
  return {
    ...
    note: 'Proof history is self-published and is not used for ranking. Key continuity is trusted only when anchored to a previously saved public key.',
  };
}
