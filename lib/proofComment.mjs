...
export function verifyProofBinding(proof, task, inputDigest, options = {}) {
  ...
  const expectedDeliveryIntent = deliveryIntentDigest(task);
  if (expectedDeliveryIntent) {
    if (proof.delivery_intent_digest !== expectedDeliveryIntent) {
      errors.push('proof delivery_intent_digest does not match task');
    }
  } else if (proof.delivery_intent_digest) {
    errors.push('proof delivery_intent_digest present for a task without delivery');
  }
  if (task.credential) {
