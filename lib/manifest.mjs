...
import { MANIFEST_FILES, ROOT_MANIFEST_FILE } from './nodeLayout.mjs';

export const MANIFEST_FILE = ROOT_MANIFEST_FILE;
...
const GITHUB_PROFILE_KEYS = new Set(['transport', 'operator']);
      ...
      ? {
          github: github
            ? {
                ...
                operator: github.operator == null ? null : String(github.operator),
...
export function validateManifest(manifest, options = {}) {
  ...
  if (manifest?.profiles?.github) {
    ...
    if (manifest.profiles.github.operator
      && !/^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})$/.test(manifest.profiles.github.operator)) {
      errors.push('profiles.github.operator must be a GitHub username');
    }
...
export async function fetchManifest(owner, repo, ref = 'main') {
  ...
  for (const path of MANIFEST_FILES) {
    ...
    if (response.ok) {
      ...
      return { text, parsed: parseManifest(text), url, path };
    }
    errors.push(`failed to fetch ${path} (${response.status}): ${url}`);
  }
