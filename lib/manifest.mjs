...
export const MANIFEST_FILE = 'CREAMLON.md';
...
const CAPABILITY_KEYS = new Set(['id', 'description', 'input', 'output']);
...
function parseYamlObject(text) {
  ...
  if (!frontMatter) {
    throw new Error(`invalid ${MANIFEST_FILE}: expected YAML front matter`);
  }
    ...
    profiles: profiles
      ? {
          ...
          authorization: authorization
            ...
            : null,
          _unknown_keys: Object.keys(profiles).filter((key) => !['github', 'authorization'].includes(key)),
        }
      : null,
