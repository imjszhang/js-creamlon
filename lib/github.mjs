...
export async function addRepositoryCollaborator(owner, repo, username, token, permission = 'push') {
  return githubRequest(`/repos/${owner}/${repo}/collaborators/${username}`, {
    method: 'PUT',
    token,
    body: { permission },
  });
...
export async function getRepositoryCollaboratorPermission(owner, repo, username, token) {
  return githubRequest(`/repos/${owner}/${repo}/collaborators/${username}/permission`, { token });
}
