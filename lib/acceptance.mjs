...
} from './extensions/delivery/schema.mjs';
...
export function validateTaskAcceptance(task, issue, options = {}) {
  ...
  const taskDelivery = parseTaskDelivery(task);
  if (taskDelivery) {
    errors.push(...validateTaskDelivery(taskDelivery, {
      manifestDelivery: parseManifestDelivery(manifest),
  ...
  if (credentialRequired || task?.credential) {
    try {
      ...
      if (credentialResult.ok) {
        ...
        if (redemption) {
          const sameRedemption = redemption.request_id === task.request_id
            && redemption.credential_digest === credentialResult.credential_digest
            && redemption.task_intent_digest === credentialResult.task_intent_digest
            && redemption.capability_id === task.capability_id;
          if (!sameRedemption) {
            credentialResult = { ok: false, reason: 'credential already redeemed' };
          }
        }
        const claim = credentialClaims?.get?.(task.credential.credential_id);
        if (credentialResult.ok && claim) {
          credentialResult = {
            ok: false,
            reason: `credential already claimed by pending task ${claim.request_id}`,
          };
        }
      }
    } catch (error) {
