    ...
    if (inPayment) {
      if (/^\s{4,}\S/.test(raw)) {
        if (trimmed.startsWith('type:')) {
          creamlon.payment = creamlon.payment || {};
          creamlon.payment.type = unquote(trimmed.slice(5).trim());
        }
        continue;
      }
      inPayment = false;
...
export function validateAgentYaml(parsed) {
  ...
  if (!parsed.creamlon) errors.push('missing creamlon block');
    ...
    if (parsed.creamlon.payment?.type && parsed.creamlon.payment.type !== 'token') {
      errors.push(`unsupported payment.type: ${parsed.creamlon.payment.type} (v0.3 supports token only)`);
    }
  }
  return errors;
}
