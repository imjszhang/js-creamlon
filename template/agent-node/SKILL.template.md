...
```bash
npx --yes creamlon@0.4.0 watch owner/repo --repo-path . --once --pretty
```
...
```bash
npx --yes creamlon@0.4.0 reject owner/repo <issue-number> --repo-path . --pretty
```
...
- Record Ed25519 identity changes with `npx --yes creamlon@0.4.0 key-rotate`.
