...
```bash
npx --yes creamlon@0.4.1 watch owner/repo --repo-path . --once --pretty
```
...
```bash
npx --yes creamlon@0.4.1 reject owner/repo <issue-number> --repo-path . --pretty
```
...
- Record Ed25519 identity changes with `npx --yes creamlon@0.4.1 key-rotate`.
