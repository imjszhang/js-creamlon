...
```bash
npx --yes creamlon@0.6.0 discover echo \
  --input-type text/plain \
  --output-type text/plain \
  --pretty

npx --yes creamlon@0.6.0 inspect owner/repo --pretty
```
...
```bash
npx --yes creamlon@0.6.0 submit owner/repo \
  --capability-id echo \
...
```bash
npx --yes creamlon@0.6.0 fetch-proof owner/repo <issue-number> \
  --verify \
  --pretty
```
...
```bash
npx --yes creamlon@0.6.0 caller inbox init --node owner/repo
npx --yes creamlon@0.6.0 caller inbox grant --node owner/repo
npx --yes creamlon@0.6.0 caller inbox protect --node owner/repo

npx --yes creamlon@0.6.0 extension delivery prepare owner/repo \
  --request-id <request_id>

npx --yes creamlon@0.6.0 extension delivery draft \
  --task-file ./task.yaml \
...
npx --yes creamlon@0.6.0 extension delivery send-input \
...
npx --yes creamlon@0.6.0 submit owner/repo --task-file ./task.yaml

npx --yes creamlon@0.6.0 fetch-proof owner/repo <issue-number> --verify --pretty

npx --yes creamlon@0.6.0 extension delivery fetch-output owner/repo <issue-number> \
  --outbox .creamlon/outbox/<request_id>.json \
...
```bash
npx --yes creamlon@0.6.0 init ./my-node --name my-node
npx --yes creamlon@0.6.0 keygen --out ./my-node/.creamlon
```
...
```bash
npx --yes creamlon@0.6.0 watch owner/repo \
  --repo-path . \
  --once \
  --pretty
```
...
```bash
npx --yes creamlon@0.6.0 reject owner/repo <issue-number> \
  --repo-path . \
  --pretty
```
